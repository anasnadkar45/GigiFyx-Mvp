...
import { Phone, Badge, Building, MapPin, Calendar, Droplet, FileText, Info, X } from "lucide-react"
...
export const SignupForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const submitData = {
        ...formData,
        role: correctRole,
        // Include documents for clinic owners
        ...(correctRole === "CLINIC_OWNER" && { documents: documentFiles }),
      }
  ...
  const handleRemoveFile = (index: number) => {
    ...
    // Clear error if files exist after removal
    if (documentFiles.length > 1 && errors.document) {
        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-col items-center gap-4">
            ...
            >
              <UploadButton
                ...
                onClientUploadComplete={(res) => {
                  ...
                  const newFiles = res.map((item) => item.url)
                ...
                appearance={{
                  button: "bg-primary text-primary-foreground hover:bg-primary/90 px-6 py-2 rounded-md",
            ...
            {documentFiles.length > 0 && (
              <div className="w-full space-y-2">
                <div className="text-sm font-medium text-green-600">
                  {documentFiles.length} file(s) uploaded successfully
                </div>
                {documentFiles.map((file, index) => (
                  <div key={index} className="flex items-center justify-between p-3 border rounded-lg bg-green-50">
                    <div className="flex items-center gap-2">
                      <FileText className="h-4 w-4 text-green-600" />
                      <span className="text-sm text-green-700">Document {index + 1}</span>
                      <a
                        href={file}
                        target="_blank"
            ...
            <p className="text-xs text-muted-foreground">
              Please upload your clinic license or registration document (PDF, JPG, PNG). Maximum 5 files, 16MB each.
            </p>
          </div>
        </div>
      </div>
